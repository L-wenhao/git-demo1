 ...
 * 
 * 扩展的两个主要功能: 1.对Properties文件中注释的保存。
 * CommentedProperties在读取和保存Properties文件时,会将其注释
 * 一起读取保存。CommentedProperties中会提供方法来根据key获取
 * 相应注释。在CommentedProperties中添加一个K-V对时，也会提供 添加相应注释的方法。
 * 
 * 2.对Properties文件中Key值顺序的保证。 CommentedProperties会保证Key的顺序。从一个Properties文件中
...
public class CommentedProperties {
	...
	private void store0(BufferedWriter bw, boolean escUnicode) throws IOException {
		synchronized (this) {
			Iterator<Map.Entry<Object, Object>> kvIter = props.entrySet().iterator();
			if(heard != null && !heard.equals(BLANK))
				writeComments(bw, heard);
			while (kvIter.hasNext()) {
				Map.Entry<Object, Object> entry = kvIter.next();
